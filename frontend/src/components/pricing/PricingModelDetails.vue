<template>
  <section class="min-w-0">
    <div v-if="model.platform_default_inbound_endpoints?.length || model.capabilities" class="mb-5 space-y-3 rounded-lg border border-white/[0.08] bg-[#111318] p-4">
    ...
    <div v-if="activeSection === 'providers'" class="space-y-4">
      ...
      <article v-for="provider in model.providers" :key="provider.name" :data-testid="`marketplace-provider-${provider.name}`" class="rounded-lg border border-white/[0.08] bg-[#111318] p-4">
    ...
    <div v-else :data-testid="`marketplace-section-empty-${activeSection}`" class="rounded-lg border border-dashed border-white/[0.14] bg-[#111318] p-8 text-center">
