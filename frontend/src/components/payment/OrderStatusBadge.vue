...
const statusMap: Record<OrderStatus, { key: string; class: string }> = {
  ...
  EXPIRED: { key: 'payment.status.expired', class: 'bg-gray-100 text-gray-800 dark:bg-dark-900/30 dark:text-gray-400' },
  CANCELLED: { key: 'payment.status.cancelled', class: 'bg-gray-100 text-gray-800 dark:bg-dark-900/30 dark:text-gray-400' },
...
const statusClass = computed(() => {
  const entry = statusMap[props.status]
  return entry?.class ?? 'bg-gray-100 text-gray-800 dark:bg-dark-900/30 dark:text-gray-400'
})
</script>
